...
namespace Blog.Web.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public UserController(IValidator<AppUser> validator, IUserService userService,IMapper mapper, IToastNotification toastNotification)
        {
        ...
        public async Task<IActionResult> Delete(Guid userId)
        {
            ...
            {
                _toastNotification.AddSuccessToastMessage(Messages.User.Delete(result.email),
                   new ToastrOptions { Title = "İşlem Başarılı" });
                return RedirectToAction("Index", "User", new { Area = "Admin" });
            }
            else
        ...
        public async Task<IActionResult> Profile(UserProfileDto userProfileDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result)
                {
                    _toastNotification.AddSuccessToastMessage("Profil güncelleme işlemi tamamlandı.",
                       new ToastrOptions { Title = "İşlem Başarılı" });
                    return RedirectToAction("Index", "User", new { Area = "Admin" });
                }
                else
                {
                    var profile = await _userService.GetUserProfileAsync();
                    _toastNotification.AddErrorToastMessage("Profil güncelleme işlemi tamamlanamadı.",
